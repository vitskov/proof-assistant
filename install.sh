...
find_terminal_editor() {
  ...
  for editor in nano pico micro; do
...
run_privileged() {
  ...
  if ! command -v sudo >/dev/null 2>&1; then
    echo "NOTE: sudo is unavailable, so the installer cannot add a terminal editor." >&2
    return 126
  fi
  ...
  if [[ -r /dev/tty && -w /dev/tty ]]; then
    echo "A terminal editor is required; sudo may prompt to install one." >&2
    if sudo -v </dev/tty >/dev/tty; then
      sudo -n "$@"
      return
    fi
  fi
  echo "NOTE: administrative access was not available for terminal-editor installation." >&2
  return 126
}
...
editor_package_for() {
  local manager="$1"
  local editor="$2"
  case "${manager}:${editor}" in
    apt-get:nano|dnf:nano|yum:nano|pacman:nano|zypper:nano|brew:nano|port:nano)
      printf 'nano\n'
      ;;
    apt-get:pico) printf 'alpine-pico\n' ;;
    dnf:pico|yum:pico|pacman:pico|zypper:pico|brew:pico|port:pico)
      printf 'alpine\n'
      ;;
    apt-get:micro|dnf:micro|yum:micro|pacman:micro|brew:micro|port:micro)
      printf 'micro\n'
      ;;
    zypper:micro) printf 'micro-editor\n' ;;
    *) return 1 ;;
  esac
}
...
install_editor_package() {
  ...
  case "${manager}" in
    apt-get)
      ...
      ;;
    dnf) run_privileged dnf install -y "${package}" ;;
    yum) run_privileged yum install -y "${package}" ;;
    pacman) run_privileged pacman --noconfirm --needed -S "${package}" ;;
    zypper) run_privileged zypper --non-interactive install "${package}" ;;
    brew) brew install "${package}" ;;
    port) run_privileged port -N install "${package}" ;;
    *) return 1 ;;
  esac
}

ensure_terminal_editor() {
  local editor_path manager="" editor package install_status
  if editor_path="$(find_terminal_editor)"; then
    echo "Using terminal editor: ${editor_path}"
    return 0
  fi
  ...
  if [[ -z "${manager}" ]]; then
    echo "ERROR: nano, pico, and micro are unavailable, and no supported package manager was found." >&2
    echo "Install nano, pico, or micro and rerun install.sh." >&2
    return 2
  fi

  echo "No supported terminal editor was found; trying nano, then pico, then micro with ${manager}."
  for editor in nano pico micro; do
    package="$(editor_package_for "${manager}" "${editor}")"
    echo "Trying to install ${editor} (${package}) with ${manager}."
    install_status=0
    if install_editor_package "${manager}" "${package}"; then
      hash -r
      if editor_path="$(find_terminal_editor)"; then
        echo "Installed terminal editor: ${editor_path}"
        return 0
      fi
    else
      install_status=$?
    fi
    if [[ "${install_status}" -eq 126 ]]; then
      echo "ERROR: Cannot install a terminal editor without the required administrative access." >&2
      echo "Install nano, pico, or micro manually and rerun install.sh." >&2
      return 2
    fi
    echo "NOTE: ${editor} installation did not produce a usable editor; continuing." >&2
  done

  echo "ERROR: Could not install nano, pico, or micro with ${manager}." >&2
  echo "Install one of those editors manually and rerun install.sh." >&2
  return 2
}
